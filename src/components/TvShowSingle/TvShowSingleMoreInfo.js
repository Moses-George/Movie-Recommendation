...
import { useGetTvShowVideosQuery } from "../../store/service/movieApiSlice";
...
const TvShowSingleMoreInfo = ({ single }) => {
    ...
    const { data: videos } = useGetTvShowVideosQuery(tvShowId);

    return (
        <div className="movie-single__moreInfo">
            <p> <strong>Rated: </strong>  {single?.adult ? "PG-18" : "PG-13"} </p>
            <p> <strong>Release: </strong> {formatDate(single?.first_air_date)} </p>
